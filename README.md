...
Также было проведено сканирование с флагом **-A**, что значит **Aggressive scan**, представляет из себя "всё в одном", и включает сразу несколько мощных проверок такие как **-sV**, **-O**, **-sC** (встроенные скрипты безопасности), а также **--traceroute** (трассировку маршрута), а проводилось оно на.. как выяснилось из результатов — основной ОС.
была приведена в действие команда:
```bash
...
1. **Роутер оказался слабым звеном**, из отчёта видно что, маршрутизатор открыто выдаёт свою ОС (Linux) и версии старых служб (Dropbear 2012), становясь лёгкой мишенью для злоумышленника.
2. **Брандмауэр Windows решает**, основная ОС успешно защитилась — статус `filtered` на 996 портах доказал, что Firewall успешно маскирует систему и не даёт завершить успешно завершить сканирование.
3. **Практика работы с сетью:** Настроена виртуальная сеть (Bridged) и освоены базовые флаги Nmap (-sV, -O, -sn, -A) для легального аудита безопасности.
